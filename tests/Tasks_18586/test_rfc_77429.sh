...
echo `date`
echo "---Test: PDP on missing Action or Resource---"
#########################################################

echo "1) Nothing to test:"
echo "This RFC is tested together with RFC 78414"
echo "OK"
echo "-------------------------------"
...
if [ $passed == "no" ]; then
echo "---Test: PDP on missing Action or Resource: TEST FAILED---"
echo `date`
exit 1
else
echo "---Test: PDP on missing Action or Resource: TEST PASSED---"
echo `date`
exit 0
fi
