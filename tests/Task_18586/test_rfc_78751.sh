...
echo `date`
echo "---Test: first authz request after service start timeout---"
#########################################################
...
if [ $passed == "no" ]; then
echo "---Test: first authz request after service start timeout: TEST FAILED---"
echo `date`
exit 1
else
echo "---Test: first authz request after service start timeout: TEST PASSED---"
echo `date`
exit 0
fi
